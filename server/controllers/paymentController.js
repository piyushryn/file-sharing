...
const initRazorpayPayment = async (req, res) => {
  ...
  try {
    ...
    console.log(
      `[${requestId}] [initRazorpayPayment] Pricing Tier:`,
      pricingTier
    );
...
const verifyRazorpayPayment = async (req, res) => {
  ...
  try {
    ...
    const updateResult = await File.findByIdAndUpdate(payment.fileId, {
...
const handleStripeWebhook = async (req, res) => {
  ...
  console.log(`[${requestId}] [handleStripeWebhook] Webhook received:`, {
    headers: {
      "stripe-signature":
        req.headers["stripe-signature"]?.substring(0, 10) + "...",
    },
  });
    ...
    if (event.type === "payment_intent.succeeded") {
      ...
      if (payment) {
        ...
        if (pricingTier && fileId) {
          ...
          const updateResult = await File.findByIdAndUpdate(fileId, {
            maxSize: pricingTier.fileSizeLimit,
            validityHours: pricingTier.validityInHours,
            isPremium: true,
            paymentId: payment._id,
