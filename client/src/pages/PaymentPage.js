...
const PaymentPage = () => {
  ...
  const [error, setError] = useState(null);
  const [paymentMethod, setPaymentMethod] = useState(null); // null, 'razorpay', or 'stripe'
      ...
      try {
        ...
        if (preselectedTierId) {
          const preselectedTier = activeTiers.find(
            (tier) => tier._id === preselectedTierId
          );
          if (preselectedTier) {
              ...
              <div>
                ...
                <PricingTierList>
                  {pricingTiers.map((tier) => (
                    <PricingTier
                      key={tier._id}
                      selected={selectedTier && selectedTier._id === tier._id}
